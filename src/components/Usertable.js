...
const TableRow = ({ item, index }) => {
  ...
  const approveorUnapprove = async () => {
    try {
      ...
      if (resp.status === 200) toast.success(res.message);
      else {
  ...
  return (
    <>
      ...
      >
        ...
        <td className="py-3 px-4 text-sm text-themeBlack-200">
          {item.nickname}
        </td>
        ...
        <td className="py-3 px-4">
          <button
            ...
            disabled={loading}
            className={`w-12 h-6 rounded-full disabled:cursor-not-allowed flex items-center p-1 px-1 transition-colors duration-300 ${
        ...
        <td className="px-4 py-1.5 flex-1 flex gap-1">
          <Link
            to={`/user/${item.nickname}/${item._id}`}
        ...
        <div className="flex gap-1 justify-between">
          ...
          <button
            onClick={(e) => {
                e.preventDefault()
                e.stopPropagation()
              approveorUnapprove(item._id)
            }}
            className={`w-10 h-5 rounded-full flex items-center p-1 px-2 transition-colors duration-300 ${
              isOn ? "bg-green-500" : "bg-gray-300"
            }`}
          >
            <div
              className={`w-3.5 h-3.5 bg-white rounded-full shadow-md transform transition-transform duration-300 ${
                isOn ? "translate-x-4" : "-translate-x-1.5"
              }`}
            />
          </button>
        </div>
